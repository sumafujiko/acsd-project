    ...
    const fetchHotels = async () => {
      try {
        ...
        if (flight && flight.outboundFlight && flight.outboundFlight.length > 0) {
          const lastFlight = flight.outboundFlight[flight.outboundFlight.length - 1];
          ...
          if (flight.returnFlight && flight.returnFlight.length > 0) {
            ...
            calculatedStayDuration = Math.ceil(
              (new Date(returnDate) - new Date(checkInDate)) / (1000 * 60 * 60 * 24)
        ...
        const adjustedHotelData = hotelData.map(hotel => ({
          ...
          offers: hotel.offers.map(offer => ({
            ...
            price: {
              ...
              total: (parseFloat(offer.price.total) * calculatedStayDuration).toFixed(2)
            }
          }))
        }));
